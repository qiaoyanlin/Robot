...
public class Trie
{
	...
	private TrieNode backAdd(TrieNode record, String temp, int now_count)
	{
		...
		{
			record.getFather().getSon()
					.add(new TrieNode(temp, record.getFather()));
			return record.getFather().getSon()
					.get(record.getFather().getSon().size() - 1);
		} else
		{
	...
	public String[] query(Word[] word) throws Exception
	{
		...
		{
			return new String[]
			{ "我可以表示无言以对嘛。", "麻烦说的清楚一点，我听不懂你在说什么", "我们聊点别的什么，好不？",
					"亲爱的，不明白你是什么意思，麻烦换一种说法", "你什么情况？为什么这样说", "先回答我的问题，你爱我么？" };
			// return matchFail(word);
		}
		return tempNode.getAnswer();
	}

	public String[] matchFail(Word[] word, FenciTest fenciTest)
			throws Exception
	{
		return learn(word, null, getRoot(), 0, fenciTest);
	}

	private String[] learn(Word[] word, TrieNode root,
			ArrayList<TrieNode> arrayList, int version, FenciTest fenciTest)
			throws Exception
	{
		try
		{
			if (root == null)
			{
				...
				{
					return learn(word, arrayList.get(Trie.getPosition()),
							arrayList.get(Trie.getPosition()).getSon(),
							++version, fenciTest);
				} else
				{
					...
					// +"\nword : ");
					// Tool.print(word);
					return createTrieNode(word, null, getRoot(), version, true,
							fenciTest);
				}
			} else
			{
				...
				} else
				{
					// Tool.getPrintWriter().println("learn() : Run the root !=
					// null and createTrieNode() ");
					return createTrieNode(word, root, arrayList, version,
							false, fenciTest);
				}
			}
		} catch (Exception e)
	...
	private boolean matchBodyString(Word[] word, TrieNode root,
			ArrayList<TrieNode> arrayList, int version) throws Exception
	{
	...
	private String[] createTrieNode(Word[] word, TrieNode root,
			ArrayList<TrieNode> arrayList, int version, boolean flag,
			FenciTest fenciTest) throws Exception
	{
		if (root == null)
		{
			...
			// +"]" + " " + word[i].getWord());
			// }
			arrayList.add(new TrieNode(word[version].getWord(), null, integers,
					fenciTest.getAnswer()));
			...
			return version == word.length - 1 ? fenciTest.getAnswer()
					: createTrieNode(word, arrayList.get(arrayList.size() - 1),
							arrayList.get(arrayList.size() - 1).getSon(),
							++version, flag, fenciTest);
		} else if (arrayList.size() != 0)
		{
			...
			arrayList.add(new TrieNode(word[version].getWord(), root, integers,
					fenciTest.getAnswer()));
			// for (int i = 0; i < integers.size(); i++) {
			// Tool.getPrintWriter()
			// .println("CreateTrieNode () :The integer of " + i + " is " +
		...
		} else
		{
			...
			Tool.writeInFile(word, root, arrayList, version, flag);
			return version == word.length - 1 ? fenciTest.getAnswer()
					: createTrieNode(word, arrayList.get(0), arrayList.get(0)
							.getSon(), ++version, flag, fenciTest);
		}
	}
	...
	private TrieNode matchTheWord(String tempString, TrieNode tempNode)
	{
		if (tempNode == null)
		{
			for (int i = 0; i < getRoot().size(); i++)
			{
				if (tempString.hashCode() == getRoot().get(i).getInfo()
						.hashCode())
				{
		...
		} else
		{
			...
			{
				if (tempString.hashCode() == tempNode.getSon().get(i).getInfo()
						.hashCode())
				{
